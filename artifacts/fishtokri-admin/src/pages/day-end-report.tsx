...
function InvoiceModal({ order, onClose }: { order: any; onClose: () => void }) {
  ...
  const handlePrint = async () => {
    ...
    const notesRow = order.notes ? `<div style="font-size:12px;margin:2px 0;"><b>Note: ${order.notes}</b></div>` : "";
    const slotLabel = order.isExpress ? "Express order by Porter" : formatTimeSlot(order);
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${invoiceNo}</title><style>* { margin:0;padding:0;box-sizing:border-box; } body { font-family:Arial,sans-serif;color:#111;background:#fff; } @page { size:80mm auto;margin:0; }</style></head><body><div style="padding:4px 8px;font-size:13px;color:#111;"><h2 style="text-align:center;font-size:16px;font-weight:700;margin-bottom:2px;">Atha Foods (Fishtokri)${order.subHubName?` - ${order.subHubName}`:""}</h2><div style="border-top:1px dashed #999;margin:8px 0;"></div><div style="text-align:center;font-size:12px;color:#555;margin-bottom:8px;">Mobile No: ${order.phone||"—"}</div><div style="display:flex;justify-content:space-between;margin:3px 0;font-size:12px;"><span><b>Invoice No:</b> ${invoiceNo}</span><span><b>Date:</b> ${deliveryDateStr}</span></div><div style="display:flex;justify-content:space-between;align-items:center;margin:3px 0;font-size:12px;"><span><b>Payment Mode:</b> ${payMode} <span style="margin-left:5px;font-size:10px;font-weight:700;text-transform:uppercase;padding:1px 6px;border-radius:20px;border:1px solid ${payStatusColor};color:${payStatusColor};background:${payStatusBg};">${payLabel}</span></span><span><b>Time:</b> ${timeStr}</span></div><div style="border-top:1px dashed #999;margin:8px 0;"></div><div style="font-size:12px;margin:2px 0;"><b>Name:</b> ${order.customerName}</div>${order.address?`<div style="font-size:12px;margin:2px 0;"><b>Add :</b> ${order.address}</div>`:""} ${slotLabel?`<div style="font-size:12px;margin:2px 0;"><b>Delivery Slot:</b> ${slotLabel}</div>`:""}`+notesRow+`<div style="border-top:1px dashed #999;margin:8px 0;"></div><table style="width:100%;border-collapse:collapse;font-size:12px;margin:4px 0;"><thead><tr style="border-bottom:1px solid #555;"><th style="padding:5px 4px;text-align:left;font-weight:600;">Item</th><th style="padding:5px 4px;text-align:right;font-weight:600;">Qty</th><th style="padding:5px 4px;text-align:right;font-weight:600;">Rate</th><th style="padding:5px 4px;text-align:right;font-weight:600;">Amt</th></tr></thead><tbody>${itemRows}</tbody></table><div style="border-top:1px dashed #999;margin:8px 0;"></div><table style="width:100%;font-size:12px;"><tr><td colspan="3"><b>Total Items: ${items.length}</b></td><td style="text-align:right;"><b>${subtotal.toFixed(2)}</b></td></tr>${discountRows}${slotRow}${delivRow}</table><div style="border-top:1px dashed #999;margin:8px 0;"></div><div style="display:flex;justify-content:space-between;font-size:15px;font-weight:700;"><span>Grand Total:</span><span>${grandTotal.toFixed(2)}</span></div>${walletRow}${paidDueRow}<div style="text-align:center;font-size:12px;color:#555;margin-top:12px;">Thank you for your business!<br/>For any query - 9220200100</div></div></body></html>`;
      ...
      <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden" style={POPPINS}>
        ...
        <div className="max-h-[70vh] overflow-y-auto p-5 bg-gray-50">
          <div className="bg-white max-w-md mx-auto p-5 text-[13px] text-gray-800 shadow-sm border border-gray-200 rounded" style={POPPINS}>
            <h3 className="text-center font-bold text-[15px] mb-1">Atha Foods (Fishtokri){order.subHubName ? ` - ${order.subHubName}` : ""}</h3>
            ...
            {(order.isExpress || formatTimeSlot(order)) && <div className="text-[12px]"><b>Delivery Slot:</b> {order.isExpress ? "Express order by Porter" : formatTimeSlot(order)}</div>}
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                ...
                {couponAmt === 0 && extraDiscAmt === 0 && discount > 0 && <tr><td className="py-1" colSpan={3}>Discount :</td><td className="py-1 text-right">- {discount.toFixed(2)}</td></tr>}
                {slotCharge>0&&<tr><td className="py-1" colSpan={3}>Slot Charge :</td><td className="py-1 text-right">+ {slotCharge.toFixed(2)}</td></tr>}
                {deliveryCharge>0&&<tr><td className="py-1" colSpan={3}>{order.isExpress ? "Porter Charge" : "Delivery Charge"} :</td><td className="py-1 text-right">+ {deliveryCharge.toFixed(2)}</td></tr>}
              </tbody>
            </table>
            <div className="border-t border-dashed border-gray-400 my-2" />
